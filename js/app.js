...
(function defineAppController() {
  ...
  function buildFavoritesReportHtml(favorites, recommendations) {
    ...
    var compareRows = favorites
      .map(function (business) {
        ...
        return (
          ...
          "<div class='chart__label'>" + escapeForReport(business.name) + "</div>" +
          "<div class='chart__track'><div class='chart__bar' style='width:" +
          percent.toFixed(1) + "%'></div></div>" +
          "<div class='chart__val'>" + escapeForReport(value) + "</div>" +
          "</div>"
        );
      })
      .join("");
    ...
    var compareChart =
      ...
      "<div class='chart'>" +
      compareRows +
      "<div class='chart__cap'>Average rating — bars are scaled to 5 stars.</div>" +
      "</div>";
    ...
    return (
      ...
      ".chart{background:rgba(255,255,255,0.93);border-radius:12px;padding:16px 18px;" +
      "box-shadow:0 10px 26px rgba(40,70,110,0.14);}" +
      ".chart__row{display:grid;grid-template-columns:150px 1fr 40px;align-items:center;" +
      "gap:12px;margin:9px 0;}" +
      ".chart__label{font-size:12px;font-weight:700;color:#2c3e50;overflow:hidden;" +
      "text-overflow:ellipsis;white-space:nowrap;}" +
      ".chart__track{height:18px;background:#e7eef6;border-radius:9px;overflow:hidden;}" +
      ".chart__bar{height:100%;min-width:2px;border-radius:9px;" +
      "background:linear-gradient(90deg,#9cc1e2,#3a5d80);}" +
      ".chart__val{font-size:12px;font-weight:800;color:#33597d;text-align:right;}" +
      ".chart__cap{font-size:11px;color:#6c8298;margin-top:10px;}" +
      "</style></head><body>" +
      "<div class='bg'></div>" +
