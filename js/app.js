...
(function defineAppController() {
  ...
  function buildFavoritesReportHtml(favorites, recommendations) {
    ...
    return (
      ...
      "body{font-family:Arial,Helvetica,sans-serif;color:#1f2421;margin:0;padding:0 0 40px;" +
      "background:#f4f6ff;-webkit-print-color-adjust:exact;print-color-adjust:exact;}" +
      ".band{background:linear-gradient(135deg,#6a82d8,#a7b3de);color:#fff;padding:26px 40px;}" +
      ".band__mark{font-size:26px;font-weight:800;letter-spacing:-0.01em;}" +
      ".band__mark i{font-style:italic;color:#eef1ff;}" +
      ".band__title{font-size:20px;margin-top:10px;font-weight:700;}" +
      ".band__meta{margin-top:4px;font-size:13px;color:#eef1ff;}" +
      ".content{padding:28px 40px 0;}" +
      ".watermark{position:fixed;bottom:30px;right:24px;font-size:130px;opacity:0.06;" +
      "transform:rotate(-18deg);pointer-events:none;}" +
      "h2{font-size:16px;margin:28px 0 8px;color:#33356a;}" +
      "table{border-collapse:separate;border-spacing:0;width:100%;background:#fff;" +
      "border-radius:12px;overflow:hidden;box-shadow:0 8px 22px rgba(40,50,110,0.10);}" +
      "th,td{border-bottom:1px solid #e6e9f7;padding:10px 12px;text-align:left;font-size:13px;}" +
      "th{background:#e6eaff;color:#33356a;}" +
      "tr:last-child td{border-bottom:none;}" +
      "ol.recs{margin:8px 0 0;font-size:14px;line-height:1.8;background:#fff;border-radius:12px;" +
      "padding:14px 16px 14px 36px;box-shadow:0 8px 22px rgba(40,50,110,0.10);}" +
      "p.recs-note{color:#5d655e;font-size:13px;}" +
      "footer{margin:30px 40px 0;color:#8a8fb0;font-size:12px;border-top:1px solid #d8ddf2;padding-top:12px;}" +
      "@media print{body{background:#fff;}.band{border-radius:0;}}" +
      "</style></head><body>" +
      "<div class='watermark'>🏪</div>" +
      "<div class='band'>" +
      "<div class='band__mark'><i>Local</i> Lift</div>" +
      "<div class='band__title'>My Favorite Local Businesses</div>" +
      "<div class='band__meta'>Lake Forest, Illinois · Prepared for " + escapeForReport(owner) +
